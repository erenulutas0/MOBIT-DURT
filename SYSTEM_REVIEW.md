...
- **Gaps found in the actual vault:** no `tags:`, no Dataview inline fields, no index/MOC pages; directory layout is inconsistent (`{year}/{unit}/{org}` *and* `{unit}/{year}/{org}` coexist, plus `BEDAS` vs `BEDAŞ` duplicate org dirs and double-nested `unclassified/unclassified`); extracted facts/summaries/risk **never reach the vault** (DB-only); older notes miss the `internal_unit` field the current template emits.
- Mobile KG: `buildKnowledgeGraphData` util (pure, tested) + inline SVG component with touch pan/zoom. Edges are purely structural (org-name string matching, `tender_id.split("-")[0]`); caps 12 companies/10 rooms/14 docs; no fact/price/risk data feeds it.

### Recommendations
- **Must:** Normalize path generation (single canonical `{year}/{unit}/{org}` + Turkish-char slug normalization for org dirs) + a one-shot re-file migration tool for the existing 44 notes. Fix now while the vault is small.
- **Should:** Enrich templates — `tags: [tender, {unit}, {org}, {document_type}]`, `internal_unit` backfill, and write extraction results into doc notes (facts table, summary section, risk level between managed markers, same AUTO-block pattern already proven). Generate index pages: `ihaleler/{org}/_index.md` and `{year}/_index.md` with Dataview `TABLE` queries (works in Obsidian; render statically in the web view later).
- **Should (KG):** New edge rules from real metadata: doc—doc edges by shared `document_type` + same org (spec ↔ contract of same tender), note—company edges from frontmatter, edge strength from shared-tag count. Keep caps but make them configurable.
- **Could (web port):** `knowledgeGraph.ts` is already a pure util — lift it to a shared module verbatim; rebuild only the rendering component for mouse/wheel interactions (the SVG markup is largely portable). Build it as a standalone page component per Area 4 rules.
...
| 4 — Task system (2–3 wks) | Subtasks, dependencies, OVERDUE-as-flag conversion, calendar view (Gantt later) | 3 for UI |
| 5 — Vault & KG (1–2 wks) | 🔶 Path work done 2026-07-09: canonical `{year}/{UNIT}/{ORG}` with Turkish transliteration in `TenderVaultWriter.vaultSegment` (+`TenderVaultWriterTest`), tags in both templates, doc notes carry year/unit/org frontmatter; `scripts/migrate-vault-layout.mjs` (dry-run default, `--apply` = backup + move/merge/backfill/index) executed — 17 tenders canonical, BEDAS/BEDAŞ under one org dir, duplicate UNCLASSIFIED-2026-06-05 merged, `_index.md` Dataview pages generated, backups at repo root `vault_backup_*`. **Open:** facts-into-notes, KG edge rules, web KG port. | 3 for web UI |
